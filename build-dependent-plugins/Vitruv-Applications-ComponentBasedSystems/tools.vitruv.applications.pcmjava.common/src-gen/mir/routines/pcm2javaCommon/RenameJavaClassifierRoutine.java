...
public class RenameJavaClassifierRoutine extends AbstractRepairRoutineRealization {
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public ConcreteClassifier getElement1(final NamedElement classSourceElement, final org.emftext.language.java.containers.Package containingPackage, final String className, final CompilationUnit compilationUnit, final ConcreteClassifier javaClassifier) {
    ...
    public void update0Element(final NamedElement classSourceElement, final org.emftext.language.java.containers.Package containingPackage, final String className, final CompilationUnit compilationUnit, final ConcreteClassifier javaClassifier) {
      JavaContainerAndClassifierUtil.updateName(javaClassifier, StringExtensions.toFirstUpper(className));
    }
    
    public CompilationUnit getElement2(final NamedElement classSourceElement, final org.emftext.language.java.containers.Package containingPackage, final String className, final CompilationUnit compilationUnit, final ConcreteClassifier javaClassifier) {
    ...
    public void update1Element(final NamedElement classSourceElement, final org.emftext.language.java.containers.Package containingPackage, final String className, final CompilationUnit compilationUnit, final ConcreteClassifier javaClassifier) {
      ...
      boolean modified = JavaContainerAndClassifierUtil.updateNamespaces(compilationUnit, newNamespaces);
      final String newName = JavaContainerAndClassifierUtil.getCompilationUnitName(containingPackage, StringExtensions.toFirstUpper(className));
      modified = (JavaContainerAndClassifierUtil.updateName(compilationUnit, newName) || modified);
      if (modified) {
  ...
  public RenameJavaClassifierRoutine(final RoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final NamedElement classSourceElement, final org.emftext.language.java.containers.Package containingPackage, final String className) {
  ...
  protected boolean executeRoutine() throws IOException {
    if (getLogger().isTraceEnabled()) {
    	getLogger().trace("Called routine RenameJavaClassifierRoutine with input:");
    	getLogger().trace("   classSourceElement: " + this.classSourceElement);
