...
public class ChangeInterfaceMethodReturnTypeRoutine extends AbstractRepairRoutineRealization {
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public InterfaceMethod getElement1(final InterfaceMethod interfaceMethod, final DataType returnType, final Optional<org.emftext.language.java.classifiers.Class> returnTypeClass) {
  ...
  public ChangeInterfaceMethodReturnTypeRoutine(final RoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final InterfaceMethod interfaceMethod, final DataType returnType) {
  ...
  protected boolean executeRoutine() throws IOException {
    if (getLogger().isTraceEnabled()) {
    	getLogger().trace("Called routine ChangeInterfaceMethodReturnTypeRoutine with input:");
    	getLogger().trace("   interfaceMethod: " + this.interfaceMethod);
    	getLogger().trace("   returnType: " + this.returnType);
    }
