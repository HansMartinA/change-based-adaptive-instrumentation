...
public class DeleteParameterRoutine extends AbstractRepairRoutineRealization {
  ...
  public DeleteParameterRoutine(final RoutinesFacade routinesFacade, final ReactionExecutionState reactionExecutionState, final CallHierarchyHaving calledBy, final OperationSignature signature, final Parameter parameter) {
  ...
  protected boolean executeRoutine() throws IOException {
    if (getLogger().isTraceEnabled()) {
    	...
    	getLogger().trace("   signature: " + this.signature);
    	getLogger().trace("   parameter: " + this.parameter);
    }
    ...
    Stream.of(new Object[] {signature, parameter, interfaceMethod, javaParameter})
    	.filter(it -> it instanceof EObject).map(it -> (EObject) it).forEach(accessibleElement ->
    		removeCorrespondenceBetween(userExecution.getElement1(signature, parameter, interfaceMethod, javaParameter), accessibleElement, null));		
