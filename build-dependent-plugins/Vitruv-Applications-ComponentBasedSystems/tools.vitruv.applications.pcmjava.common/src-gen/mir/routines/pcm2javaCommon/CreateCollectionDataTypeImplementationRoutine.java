...
public class CreateCollectionDataTypeImplementationRoutine extends AbstractRepairRoutineRealization {
  ...
  private static class ActionUserExecution extends AbstractRepairRoutineRealization.UserExecution {
    ...
    public void callRoutine1(final CollectionDataType dataType, final Optional<org.emftext.language.java.classifiers.Class> innerTypeClass, final org.emftext.language.java.containers.Package datatypesPackage, @Extension final RoutinesFacade _routinesFacade) {
      ...
      final int selectedType = (this.executionState.getUserInteractor().getSingleSelectionDialogBuilder().message(selectTypeMsg).choices(collectionDataTypeNames).windowModality(UserInteractionOptions.WindowModality.MODAL).startInteraction()).intValue();
      ...
      _routinesFacade.createOrFindDataTypeClass(dataType, datatypesPackage);
  ...
  protected boolean executeRoutine() throws IOException {
    if (getLogger().isTraceEnabled()) {
    	getLogger().trace("Called routine CreateCollectionDataTypeImplementationRoutine with input:");
