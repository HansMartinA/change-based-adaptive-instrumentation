...
public class CreatedCompositeDataTypeReaction extends AbstractReactionRealization {
  private CreateEObject<CompositeDataType> createChange;
  
  private InsertEReference<Repository, CompositeDataType> insertChange;
  
  private int currentlyMatchedChange;
  
  public CreatedCompositeDataTypeReaction(final RoutinesFacade routinesFacade) {
  ...
  public void executeReaction(final EChange change) {
    ...
    mir.reactions.pcm2javaCommon.CreatedCompositeDataTypeReaction.ActionUserExecution userExecution = new mir.reactions.pcm2javaCommon.CreatedCompositeDataTypeReaction.ActionUserExecution(this.executionState, this);
  ...
  private boolean matchCreateChange(final EChange change) {
    if (change instanceof CreateEObject<?>) {
    	CreateEObject<org.palladiosimulator.pcm.repository.CompositeDataType> _localTypedChange = (CreateEObject<org.palladiosimulator.pcm.repository.CompositeDataType>) change;
    	if (!(_localTypedChange.getAffectedEObject() instanceof org.palladiosimulator.pcm.repository.CompositeDataType)) {
    		return false;
    	}
    	this.createChange = (CreateEObject<org.palladiosimulator.pcm.repository.CompositeDataType>) change;
    	return true;
    }
